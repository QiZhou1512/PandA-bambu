      ...
      if(fu_curr->CM)
      {
         ...
         else
         {
            ...
            for(unsigned int i = 0; i < n_ports; ++i)
            {
               ...
               if(port_c && (port_c->get_id() == CLOCK_PORT_NAME || port_c->get_id() == RESET_PORT_NAME || port_c->get_id() == START_PORT_NAME || port_c->get_id() == CLOCK_GATING_PORT_NAME ||
                             (GetPointer<port_o>(port_c) && GetPointer<port_o>(port_c)->get_is_memory()) || port_c->get_id().find("sel_") == 0))
                  continue;
                     ...
                     if(isTemplate)
                     {
                        for(unsigned int iport = 0; iport < n_ports; ++iport)
                        {
                           ...
                           if(port->get_id() == CLOCK_PORT_NAME || port->get_id() == RESET_PORT_NAME || port->get_id() == START_PORT_NAME || port->get_id() == CLOCK_GATING_PORT_NAME ||
                              (GetPointer<port_o>(port) && GetPointer<port_o>(port)->get_is_memory()) || port->get_id().find("sel_") == 0)
                              continue;
